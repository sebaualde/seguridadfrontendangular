...
import { SecurityService, userResetPassDTO } from 'src/app/Security';
import { PasswordMatch } from '../registro/Validators/PasswordMatch';
...
export class ResetPasswordComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    let credenciales: userResetPassDTO = {
      email: this.form.get('email')!.value,
      password: this.form.get('password')!.value,
      confirm: this.form.get('confirm')!.value,
      token: this.tokenRestore
    };
    ...
    this.seguridadService
      ...
      .subscribe({
        next: () => {
          this.selectPage(this.routes.Login + '/Password reseteado con exito!!!');
  ...
  normalizeToken(){
    const urlToken = this.activatedRoute.snapshot.queryParams['token'];
    this.tokenRestore = urlToken.replace(/\s/g, "+");
  }
